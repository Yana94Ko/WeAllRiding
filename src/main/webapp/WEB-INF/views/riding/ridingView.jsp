		...
		<div class="containerWrap">
			<div class="ridingViewContainer">
				<form method='post'>
					...
					<ul>
						<div id="ridingViewTitle" style="display:none;">라이딩 뷰${resolveStatus }</div><br><br>
						<h2 id="riding_header"></h2>
							<div id="titleHeader">
								<input type="hidden" name="ridingSubject" value="${vo.ridingSubject }">${vo.ridingSubject }<br>
							</div>
							<ul>
								...
								<ul id="eList">
									...
									<c:forEach var="vo" items="${lst2 }"  varStatus="st">
										<c:if test="${vo.ridingNo == vo.ridingNo}">
											<li>
												<span id = "forWriterRidingState${st.index}">
													<a href = "/member/loginForm">열람하기</a>
												</span>
												<input type="hidden" id = "dbForWriterRidingState${st.index}" value="${vo.ridingState}"/>
											</li>
											<li>${vo.nickname }</li>
											<li>${vo.gender }</li>
				...
				<div id="listHeader">
					...
					<c:if test="${logStatus=='Y' }">
						<form method='post' id="ridingReplyFrm">
							...
							<input type="button" value="댓글등록" id="replyBtn" class="ridingReplyBtn" onclick="ridingReplyFrm()">
						</form>
					</c:if>
					<!-- 댓글목록이 나올 자리 -->
