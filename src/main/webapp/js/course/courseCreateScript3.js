...
function searchAllCourse(){
	...
	//--------> 경로 정보 표시 관련 Start <-------
	$(".rourouteInfoWrap div").css("display","inline-block");
...
function searchCourse(preference, routeNo) {
	...
	request.onreadystatechange = function() {
		if (this.readyState === 4) {
			...
			$("#routeDistance"+routeNo).html(distances[routeNo]);
			durationes[routeNo]=(course[0].summary.duration/60).toFixed(0);//초를 분단위로 변환
			$("#routeDuration"+routeNo).html(durationes[routeNo]);
			/*ascents[routeNo]=course[0].summary.ascent;
			$("#routeAscent"+routeNo).html(ascents[routeNo]);
			descents[routeNo]=course[0].summary.descent;
			$("#routeDescent"+routeNo).html(descents[routeNo]);*/
...
/*			console.log(preference+"의 상승고도" + ascents[routeNo]);
			...
			maxElevation[routeNo] =Math.max.apply(Math, chartData);
			minElevation[routeNo] =Math.min.apply(Math, chartData);
			console.log(preference+"의 최대고도" + maxElevation[routeNo]);
			console.log(preference+"의 최저고도" + minElevation[routeNo]);
			$("#routeAscent"+routeNo).html(maxElevation[routeNo]);
			$("#routeDescent"+routeNo).html(minElevation[routeNo]);
...
function setCourseLineTest(points, preference, routeNo){	
	...
	if (routeNo == 0) {
		...
		generateThumbnail(polyline0, linepath0);
	} else if (routeNo == 1) {
		$.each(points, function(index, v) {
			var p = new kakao.maps.LatLng(v[0], v[1]);
			linepath1.push(p);
