...
var radiuses = "200";// 각 웨이포인트마다 주변 도로를 검색할 범위 제한(m) / -1 : 최대 검색 반경 사용 / 값을 1개만 입력시 모든 웨이포인트에 적용
...
function searchCourse(preference, cnt) {
	...
	request.onreadystatechange = function() {
		if (this.readyState === 4) {
			...
			for (var i = 0; i < points.length; i++) {
				arr_label.push('"' + points[i][0] + ',' + points[i][1] + '"');
				arr_data.push(points[i][2]);
			}
			var myChart;
			//차트 그리기
			//plot(arr_label, arr_data);
			
			//경로 생성
			//setCourseLine(points);
			console.log(cnt);
			setCourseLineTest(points, preference, cnt);
...
// 경로 라인 생성하기
var polyline = "";
var linepath = [];//경로를 만들 지점들 좌표의 배열
...
//------------------------> 경로라인 생성 관련 end <------------------------
//-------------------------> 경로삭제 관련 start <-------------------------
/*function clearPolyline() {//삭제버튼 클릭시, deleteAllWaypoint(), removeAllMarkers() 통해 좌표와 마커 삭제.
	if (polyline != "") {
...
//--------------------------> 경로삭제 관련 end <--------------------------
//========================================> 경로 생성/삭제 관련 end <=============================================
//========================================> 다중 경로 생성/삭제 관련 start <=============================================
//다중 경로 생성
var nowPreference = "";


var linepathRecommend = [];
var linepathFastest = [];
var linepathShortest = [];

//객체 선언
var polylineRecommend = new kakao.maps.Polyline({
		path : linepathRecommend,
		strokeWeight: 6, // 선의 두께 입니다
		strokeColor: '#ff0000', // 선의 색깔입니다 빨강
		strokeOpacity: 3, // 선의 불투명도 입니다 투명합니다
		strokeStyle: 'solid' // 선의 스타일입니다
});

var polylineFastest = new kakao.maps.Polyline({
		path : linepathFastest, 
		strokeWeight: 6, // 선의 두께 입니다
		strokeColor: '#0000ff', // 선의 색깔입니다 파랑
		strokeOpacity: 0.8, // 선의 불투명도 입니다 투명합니다
		strokeStyle: 'solid' // 선의 스타일입니다
});
var polylineShortest = new kakao.maps.Polyline({
		path : linepathShortest,
		strokeWeight: 6, // 선의 두께 입니다
		strokeColor: '#ffd800', // 선의 색깔입니다 노랑
		strokeOpacity: 0.8, // 선의 불투명도 입니다 투명합니다
		strokeStyle: 'solid' // 선의 스타일입니다
});
...
function searchAllCourse(){
	var cnt = 0;
	clearAllPolylines();
	$.each(preferences, function(cnt, value){ 
		preference = value;
		searchCourse(preference, cnt);
	});
}

//라인 생성
function setCourseLineTest(points, preference, cnt){	
	console.log("setCourseLineTest : 조건문 들어가기 전 " +preference +"cnt"+ cnt);
	if(preference ==="recommended"){
		console.log("현재 속성은 : " +preference);
		$.each(points, function(index, v) {
			var p = new kakao.maps.LatLng(v[0], v[1]);
			linepathRecommend.push(p);
		});	
		polylineRecommend.setPath(linepathRecommend);
		polylineRecommend.setMap(map);
	} else if(preference ==="fastest"){
		console.log("현재 속성은 : " +preference);
		$.each(points, function(index, v) {
			var p = new kakao.maps.LatLng(v[0], v[1]);
			linepathFastest.push(p);
		});	
		polylineFastest.setPath(linepathFastest);
		polylineFastest.setMap(map);
	} else if(preference ==="shortest"){
		console.log("현재 속성은 : " +preference);
		$.each(points, function(index, v) {
			var p = new kakao.maps.LatLng(v[0], v[1]);
			linepathShortest.push(p);
		});	
		polylineShortest.setPath(linepathShortest);
		polylineShortest.setMap(map);
	}
...
function selectRecommend(){
		...
		polylineRecommend.setOptions({
		    strokeColor: '#ff0000',
		    strokeOpacity: 0.8,
		});
		polylineFastest.setOptions({
		    strokeColor: '#9d9d9d',
		    strokeOpacity: 0.8,
		});
		polylineShortest.setOptions({
		    strokeColor: '#9d9d9d',
		    strokeOpacity: 0.8,
		});
		polylineRecommend.setZIndex(3);
		polylineShortest.setZIndex(1);
		polylineFastest.setZIndex(1);
		polylineShortest.setMap(map);
		polylineFastest.setMap(map);
		polylineRecommend.setMap(map);
}
...
function selectFastest(){
		...
		polylineRecommend.setZIndex(1);
		polylineShortest.setZIndex(1);
		polylineFastest.setZIndex(3);
